...
void Server::action() {
    std::vector<pollfd> eventsToProcess;
    for (size_t i = 0; i < _pollfds.size(); ++i) {
        const pollfd& curPollfd = _pollfds[i];
...
void Server::handlePollHup(const pollfd& curPollfd) {
    for (std::vector<pollfd>::iterator itPollfd = _pollfds.begin(); itPollfd != _pollfds.end(); ++itPollfd) {
        if (itPollfd->fd == curPollfd.fd) {
            size_t userIndex = std::distance(_pollfds.begin(), itPollfd) - 1;
            if (userIndex < _users.size()) {
...
void Server::handleClientData(const pollfd& curPollfd) {
    for (std::vector<User*>::iterator itUser = _users.begin(); itUser != _users.end(); ++itUser) {
        std::vector<pollfd>::iterator itPollfd = _pollfds.begin() + std::distance(_users.begin(), itUser) + 1;
...
int				Server::recvMsg(User *user) {
	...
	{
		memset(message, '\0', sizeof(message));
		byteRecved = recv(user->getSockFd(), message, sizeof(message), 0);
		if (byteRecved <= 0)
			break ;
